    ...
    [
      {
        ...
        settings: [
          ...
          ['恨みダメージ', '/残りHPが少ないほど(.*?)。/'],
          ['固定ダメージ', '/(敵.+?)の?固定/'],
          ['残りHP割合ダメージ', '/(?:敵の残りHPが(\\d+％)減少|敵のHPが(\\d+％))/'],
          ['最大HP割合ダメージ', '/(?:敵のHP(\\d+％分)のダメージ|敵の最大HP(\\d+％分))/']
        ]
      },
      {
        label: 'ドロップ変換',
      ...
      },
      {
        label: '回復',
        settings: [
          ['HP回復', '/(?:HP.?(\\d+回復)|[^、]HPを(\\d+％)回復|最大HPの?(\\d+％)分の?HP回復|[^、]HP[^。回]*(全回復))/'],
          ['HP自動回復', '/(?:(\\d+ターンの間、HPを?\\d+％)回復|(\\d+ターンの間、最大HPの?\\d+％)分回復)/'],
          ['バインド状態回復', '/バインド[^。]*を(.*?回復)/'],
          ['覚醒無効状態回復', '/覚醒無効[^。]*を(.*?回復)/'],
          ['消せないドロップ状態回復', '/消せないドロップ[^。]*を(.*?回復)/']
        ]
      },
      {
        label: 'エンハンス',
        settings: [
          ['タイプエンハ', '/の間[^。]*、(?:回復力と)?\\n?(.*タイプ)の攻撃力が(?:[\\d.]+倍|.*上昇|.*上がる)/'],
          ['属性エンハ', '/の間[^。].*?([火水木光闇][火水木光闇と]*属性)の攻撃力が(?:[\\d.]+倍|.*上昇|.*上がる)/'],
          ['覚醒エンハ', '/の間、チーム内の(.+)の.*攻撃力/'],
          ['回復力エンハ', '/回復力[^。、]*?が([\\d\\.]+倍)/'],
          ['回復力減少', '/(?:回復力.*?が([^、。]+減)[、。]|回復力.*?が(0)に)/']
        ]
      },
      {
        label: '自分対象補助',
      ...
      },
      {
        ...
        settings: [
          ...
          ['毒', '/を(.*?毒)にする/'],
          ['敵属性変化', '/敵全体が(.*属性に変化)。/'],
          ['吸収無効化', '/(\\d+ターンの間、.*吸収を無効化)/', false],
          ['ダメージ吸収無効化', '/(\\d+ターン)の間、ダメージ吸収(?:と属性吸収)?を無効化/'],
          ['属性吸収無効化', '/(\\d+ターン)の間、(?:ダメージ吸収と)?属性吸収を無効化/'],
          ['ダメージ無効貫通', '/(\\dターン)の間、[^。]*ダメージ無効を貫通/']
        ]
      }
    ],
    [
