...
sub mode_update_skill {
  ...
  &to_number_with_key($data->{updateData}, qw/ baseTurn minTurn /);
  ...
  &check_range('スキル最短ターン', $data->{updateData}{minTurn}, 1, 199, 0);
...
sub set_skill_data {
  ...
  if (!$is_leader_skill) {
    push @target_columns, qw/ baseTurn minTurn /;
  }
...
sub mode_update_monster_data {
  ...
  if ($data->{skill} == 0) {
    ...
    if (!length $data->{skillDetails}{name} && !length $data->{skillDetails}{description}) {
      ...
      &to_number_with_key($data->{skillDetails}, qw/ baseTurn minTurn /);
      ...
      &check_range('スキル最短ターン', $data->{skillDetails}{minTurn}, 1, 199, 0);
    }
  }
...
sub save_monster_list_json {
  ...
  push @sitemap_url_paths, map { "/ranking/$_" } qw/ hp attack recovery plus overlimitOffset wayAttack lJiAttack 7comboAttack 10comboAttack way7conboAttack lJi7comboAttack a3x3Attack a3x37comboAttack assistHp assistAttack assistRecovery assistPlus /;
  my @sorted_data = sort { $a->{no} <=> $b->{no} } @$data_ref;
  push @sitemap_url_paths, map { '/monster/' . $_->{no} } @sorted_data;
  push @sitemap_url_paths, map { "/monster/$_->{no}/material" } grep { defined($_->{evolutionType}) && $_->{evolutionType} != 0 } @sorted_data;
